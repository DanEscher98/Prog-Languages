#include("qOpVector.jl"); using Main.QuantumStates
...
struct qState
    ...
    function qState(vector::Vector{<:Number}, name::String="_")
        if size(vector)[1]==2
            ...
            c = 1/sympy.sqrt(arg)
            return new(Sym(1)*vector, c, name)
        else
...
begin const
     ...
     σy = qGate([0 -im; im 0])
     σz = qGate([1 0; 0 -1])
     H = qGate(1/sympy.sqrt(2)* [1 1; 1 -1])
     I = qGate([1 0; 0 1])
     sOne = qState([0, 1], "1")
     sZero = qState([1, 0], "0")
     sPlus = qState(1/sympy.sqrt(2)*(sZero+sOne).vector, "+")
     sMinus = qState(1/sympy.sqrt(2)*(sZero-sOne).vector, "-")
     sIPlus = qState(1/sympy.sqrt(2)*(sZero+im*sOne).vector, "+𝑖")
     sIMinus = qState(1/sympy.sqrt(2)*(sZero-im*sOne).vector, "-𝑖")
end
